...
export function setupPortfolioMotion(root) {
    ...
    const context = gsap.context(() => {
      ...
      media.add("(prefers-reduced-motion: no-preference)", () => {
        ...
        const heroBackground = root.querySelector(".hero-poster");
        ...
        if (work) revealProjectGallery(work);
...
export function setPortfolioMotionPaused(paused) {
  gsap.globalTimeline.paused(paused);
  ScrollTrigger.getAll().forEach((trigger) => {
    if (paused) trigger.disable(false, true);
    else trigger.enable(false, true);
  });
}

export function revealProjectGallery(work) {
  if (!work || work.dataset.galleryMotionReady === "true") return;
  const projectCards = gsap.utils.toArray(".project-card", work);
  if (!projectCards.length) return;

  work.dataset.galleryMotionReady = "true";
  const projectMedia = gsap.utils.toArray(".project-media", work);
  const projectImages = gsap.utils.toArray(".project-media img", work);
